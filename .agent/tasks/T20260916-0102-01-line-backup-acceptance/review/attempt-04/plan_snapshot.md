...
PRODUCT_BOUNDARY_DECISION: The task deliverable is an operator-facing reusable local product, not an acceptance-only fixture. Its product owner is the LINE album backup task deliverable; its consumers are the documented operator CLI commands below. The CLI and library are the single real process boundary for verify-only and offline resume/commit/duplicate behavior. The formal data project is an input/state authority and remains read-only. The plan makes no claim that another absent external producer already consumes this module. Any future integration with another producer is a new architecture/contract decision and requires PLAN_REVISION 5 plus independent review.
...
- transaction prepare --state PATH --run-id ID --owner-id ID --group-key KEY --start-date ISO --end-date ISO --expected-images INT --destination PATH --evidence-dir PATH
...
- Case 07 launches two acquisition/finalization processes paused at ACQUIRE_BEFORE_LOCK on literal barrier /private/tmp/line-backup-acceptance-case-07/acquire.barrier; release once and compare one winner/one conflict.
...
2. After Rev4 approval, create the named product package/CLI and tests; prove the package is the only operator process boundary for the new reusable verifier/transaction deliverable.
...
7. Check documented CUA capability and the existing ledger. If route evidence remains necessary, request exactly one fresh controlled observation gate. This gate permits one ellipsis GUI input only, never Save-All/menu-item/chooser/state writes. A later production Save-All route requires PLAN_REVISION 5, fresh review, and a separate one-time production gate.

## Requirement, current evidence, and closure

| Requirement | Current evidence | Rev4 closure | Class | Closure gate |
|---|---|---|---|---|
...
| Independent acceptance | Rev1/Rev2/Rev3 reviews exist; Rev4 review pending | Fresh review of Rev4, then Stage 05 acceptance of actual CLI/evidence | CORE / MUST_NOT_BREAK | HARD_CLEAN |
...
| Destination outside backup_root | FAIL | NOT_RUN | NOT_RUN | NOT_ACHIEVED | NOT_ACHIEVED | 4 | INPUT_NEGATIVE | PASS |
| Special file | FAIL | NOT_RUN | NOT_RUN | NOT_ACHIEVED | NOT_ACHIEVED | 4 | INPUT_NEGATIVE | PASS |
...
A row with an internal read/command/artifact error has exact UNKNOWN overall because the verifier cannot safely conclude the requested result. No alternative exit is permitted. Filesystem PASS never implies registry/source/state PASS; registry/source-only defects retain filesystem PASS.
...
- Cases 02/03 use dispatcher-crash-after-side-effect.py or dispatcher-unknown.py and then a fresh literal resume --no-dispatch. Case 05 uses barrier /private/tmp/line-backup-acceptance-case-05/commit-ready.barrier and releases B before A. Case 07 uses barrier /private/tmp/line-backup-acceptance-case-07/acquire-ready.barrier and releases one fixed acquisition winner.
...
Independent oracle and durable evidence:
...
- New ellipsis budget is exactly 1 only after explicit authorization. The sequence is target-binding read → one current-target ellipsis input → immediate post observation → stop.
...
- Production Save-All requires PLAN_REVISION 5, fresh independent review, a newly created empty destination under /Users/hsiaojohnny/Downloads/LINE-Backup-PoC, exact group/album/count/destination gate and one atomic attempt with no retry after UNKNOWN. Existing valid 57 files may not be redownloaded.
...
| Legacy normalization without source proof | UNKNOWN | COMPLETE | PASS | PASS | PENDING | PENDING_REQUIRED_VERIFICATION | CORE_ACCEPTANCE/BLOCKED/INPUT_UNAVAILABLE |
| Contradictory state axes | NOT_ACHIEVED | COMPLETE | FAIL | NOT_RUN | PENDING | FIX_REQUIRED | CORE_ACCEPTANCE/FAIL/TASK_REGRESSION |
