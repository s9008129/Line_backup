...
PRODUCT_BOUNDARY_DECISION: The task deliverable is an operator-facing reusable local product, not an acceptance-only fixture. Its product owner is the LINE album backup task deliverable; its consumers are the documented operator CLI commands below. The CLI and library are the single real process boundary for verify-only and offline resume/commit/duplicate behavior. The formal data project is an input/state authority and remains read-only. The plan makes no claim that another absent external producer already consumes this module. Any future integration with another producer is a new architecture/contract decision and requires PLAN_REVISION 8 plus independent review.
...
2. After Rev7 approval, create the named product package/CLI and tests; prove the package is the only operator process boundary for the new reusable verifier/transaction deliverable.
...
7. Check documented CUA capability and the existing ledger. If route evidence remains necessary, request exactly one fresh controlled observation gate. This gate permits one ellipsis GUI input only, never Save-All/menu-item/chooser/state writes. A later production Save-All route requires PLAN_REVISION 8, fresh review, and a separate one-time production gate.

## Requirement, current evidence, and closure

| Requirement | Current evidence | Rev7 closure | Class | Closure gate |
|---|---|---|---|---|
...
| Independent acceptance | Rev1/Rev2/Rev3/Rev4/Rev5/Rev6 reviews exist; Rev7 review pending | Fresh review of Rev7, then Stage 05 acceptance of actual CLI/evidence | CORE / MUST_NOT_BREAK | HARD_CLEAN |
...
- Case 02 starts from a prepared pre-state revision 1 whose intent is READY, dispatch_state is NOT_DISPATCHED, dispatch_outcome is NOT_ATTEMPTED and retry permission is true. It runs `transaction resume --state /private/tmp/line-backup-acceptance-case-02/state.json --run-id RUN-CASE-02 --expected-revision 1 --expected-owner-id WRITER-CASE-02 --dispatcher /private/tmp/line-backup-acceptance-case-02/dispatcher-crash-after-side-effect.py --dispatch-counter /private/tmp/line-backup-acceptance-case-02/counter.jsonl --evidence-dir /private/tmp/line-backup-acceptance-case-02/evidence --test-mode --dispatcher-outcome RETURNED --crash-after-dispatch`; before the adapter side effect it must durably replace revision 2 with dispatch_state=DISPATCH_REQUESTED, dispatch_outcome=UNKNOWN and retry permission=false, then the injected adapter appends exactly one counter line and exits as DISPATCHER_CRASH_AFTER_SIDE_EFFECT with process exit 1. A fresh `transaction resume --state /private/tmp/line-backup-acceptance-case-02/state.json --run-id RUN-CASE-02 --expected-revision 2 --expected-owner-id WRITER-CASE-02 --dispatcher /private/tmp/line-backup-acceptance-case-02/dispatcher-crash-after-side-effect.py --dispatch-counter /private/tmp/line-backup-acceptance-case-02/counter.jsonl --evidence-dir /private/tmp/line-backup-acceptance-case-02/evidence --no-dispatch` must return RECOVERY_NO_DISPATCH exit 0, preserve revision 2 and all those fields, and not dispatch; otherwise recovery acceptance stops.
- Case 03 starts from the same literal revision-1 precondition. It runs `transaction resume --state /private/tmp/line-backup-acceptance-case-03/state.json --run-id RUN-CASE-03 --expected-revision 1 --expected-owner-id WRITER-CASE-03 --dispatcher /private/tmp/line-backup-acceptance-case-03/dispatcher-unknown.py --dispatch-counter /private/tmp/line-backup-acceptance-case-03/counter.jsonl --evidence-dir /private/tmp/line-backup-acceptance-case-03/evidence --test-mode --dispatcher-outcome UNKNOWN`; before calling the adapter it must durably replace revision 2 with dispatch_state=DISPATCH_REQUESTED, dispatch_outcome=UNKNOWN and retry permission=false, the adapter must append exactly one counter line and return DISPATCH_UNKNOWN with process exit 1. A fresh `transaction resume --state /private/tmp/line-backup-acceptance-case-03/state.json --run-id RUN-CASE-03 --expected-revision 2 --expected-owner-id WRITER-CASE-03 --dispatcher /private/tmp/line-backup-acceptance-case-03/dispatcher-unknown.py --dispatch-counter /private/tmp/line-backup-acceptance-case-03/counter.jsonl --evidence-dir /private/tmp/line-backup-acceptance-case-03/evidence --no-dispatch` must return RECOVERY_NO_DISPATCH exit 0, preserve revision 2/UNKNOWN/false and not dispatch; otherwise no retry interpretation is allowed.
...
- Case 10A runs `transaction finalize --state /private/tmp/line-backup-acceptance-case-10/verified/state.json --run-id RUN-CASE-10A --expected-revision 2 --expected-owner-id WRITER-CASE-10A --outcome VERIFIED --verification-json /private/tmp/line-backup-acceptance-case-10/verified/verification.json --evidence-dir /private/tmp/line-backup-acceptance-case-10/verified/evidence`; exact exit is 0 and the independently read replacement has verification, registry, workflow_outcome=VERIFIED, active_writer_id=null and context_lock=null.
- Case 10B runs `transaction finalize --state /private/tmp/line-backup-acceptance-case-10/safe-abort/state.json --run-id RUN-CASE-10B --expected-revision 2 --expected-owner-id WRITER-CASE-10B --outcome SAFE_ABORT --verification-json /private/tmp/line-backup-acceptance-case-10/safe-abort/verification.json --evidence-dir /private/tmp/line-backup-acceptance-case-10/safe-abort/evidence`; exact exit is 0 and the independent oracle requires revision 3, workflow_outcome=SAFE_ABORT, active_writer_id=null, context_lock=null, original intent_state/dispatch_state/trigger_outcome/dispatch_outcome and retry=false preserved, no registry addition, and terminal evidence recording the unresolved barrier and original observations. Any VERIFIED registry or cleared unresolved intent is a regression.
...
- Production Save-All requires PLAN_REVISION 8, fresh independent review, a newly created empty destination under /Users/hsiaojohnny/Downloads/LINE-Backup-PoC, exact group/album/count/destination gate and one atomic attempt with no retry after UNKNOWN. Existing valid 57 files may not be redownloaded.
