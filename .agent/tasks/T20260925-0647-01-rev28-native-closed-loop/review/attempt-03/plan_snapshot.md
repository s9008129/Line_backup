...
- PLAN_REVISION: `2`
- PLAN_REVISION_BASIS: revision 2 applies the Stage-02 review results on revision 1 (SHA-256 `5575aca5f2c8ededa3f76ff79baf6b82eecd25353457b709982eb05e6cbcbbb9`): review attempt-02 (report SHA-256 `025c74cb61ee067b49a4f6347b74d9764fb68596e8e9932e9e7e7202ded0360e`, verdict `PLAN_REVISION_REQUIRED`) findings RV-01…RV-10, plus the non-blocking minors of review attempt-01 (report SHA-256 `2ad96474f9f9274405b45bbe9e338f3491ae35006d95a35866d4a719c10b2b98`, verdict `PLAN_APPROVED`). Revision 1 bytes and both review attempt dirs are preserved unchanged; no gate is weakened (RV-01 replaces an unreviewable exact-equality rule with a validated-tolerance rule frozen by the W2 harness; RV-02 fixes the success predicate to content multisets; no identity/confirmation/recovery semantics are loosened).
- TASK_CLASS: `CRITICAL`
...
- Accepted baseline `/Users/hsiaojohnny/Downloads/LINE-Backup-PoC/album-2024-05-13_to-2024-05-17_57`: 57 files, 17,924,900 bytes; **name-inclusive manifest digest** recomputed on disk this round = `b7debe929a24406a44f53708194b644a4811559cf91ad87d5a55e5da91a28fbd` (method: sorted `relpath\tsize\tsha256` lines, SHA-256 of the concatenation) = MATCH. READ-ONLY. This digest is the baseline's identity/manifest artifact and a tripwire only — never the `DUPLICATE_CONTENT_CONFIRMED` comparison predicate (baseline filenames embed the per-download date token `LINE_ALBUM_20240513～0517_260907_*.jpg`; a fresh download must carry a new token).
...
Filesystem-tripwire attribution (no unattributed aborts): writes strictly inside the fresh staging run dir are the primary chooser-effect signal and are attributable to the download flow. New post-dispatch writes elsewhere under the approved root are attributed via {process census diff (target app / panel processes / engine), path pattern, FSEvents timing, the engine's own pre-declared writes}; attributable → recorded as supporting evidence; unattributable → `ABORTED_UNATTRIBUTED_FILESYSTEM_WRITE` (fail closed, no retry, concurrent-machine-activity caveat disclosed in the ledger). Pre-dispatch environmental activity is recorded as context and never aborts by itself: while the run sits at `SAVE_ALL_LOCATED`, the monitor records a ≥10 s pre-dispatch environmental baseline (FSEvents) for exactly this attribution.
...
- Verify chooser identity first against the **frozen chooser-affirmation predicate** (W2 freezes it before the seven reviews; the hosting shape comes from harness calibration, never from assumption):
  ...
  3. with unambiguous ownership: the owning pid is bound to the freshly established panel-process set — pre-dispatch census (target app pid + any process hosting a panel-like window in the observed recent past) plus post-dispatch census; the specific instance is authenticated by bundle identifier / code-signing identity + process start time (pid reuse rejected); an empty pre-census set is allowed, in which case ownership rests on (1)+(2)+timing;
  4. observed inside the bounded postcondition window with full capture/AX/CG evidence.
  Panel-process membership is one ingredient, never the sole gate; the predicate explicitly permits whichever hosting shape (separate panel service, in-process, or other) the harness proves on this macOS version, and that resolved shape is recorded in the frozen review set. Fake or ambiguous surfaces → refusal.
...
6. **AX chooser observation + predicate freeze (RV-09)**: AX read of the harness `NSOpenPanel` (roles/subroles/titles/buttons/path affordance), AXPress confirm path, keyboard Go-to-folder path, post-confirmation destination verification, ownership/pid-reuse semantics (bundle/signing identity + process start time; pre/post census, empty-set case) — all frozen into the chooser-affirmation predicate — plus an ambiguity fixture that must refuse and a look-alike owned by a different process that must refuse.
...
**Phase B — production run.** Counters (all others zero): Save All dispatch = 1; destination confirmation = 1; irreversible side-effecting ops = 1; no retry of any irreversible op; no historical coordinates. Reversible navigation (incl. ellipsis/menu re-open) follows `REVERSIBLE_NAVIGATION`: ≤3 attempts per identical blocker with fresh preconditions, global reversible-dispatch ceiling = 12 for the run (ledger-counted), whichever binds first — exceeding the ceiling → `ABORTED_REVERSIBLE_BUDGET_EXHAUSTED`. All live captures/OCR extracts stay in the untracked evidence dir and are never pushed.
...
No broad repository-wide gates are planned: every check below is bespoke. Agent waivers are not allowed (`WAIVER_ALLOWED: NO`; authority: the owner only). `BASELINE_REQUIRED: NO` for every item (the baseline comparison is itself the success predicate and is tracked as its own subject); `BASELINE_DELTA` is not used as a gate.

| Check | GOAL_CRITICALITY | EVIDENCE_ROLE | CLOSURE_GATE | FAILURE_CLASSIFICATION_RULE |
|---|---|---|---|---|
| Capability probe (executable context, TCC, capture/vision/actuation APIs) | CORE | DIRECT | YES — blocks Phase B | `NOT_RUN` → pending; `FAIL` → replan/blocked |
| Harness calibration incl. frozen per-state capture rule | CORE | DIRECT | YES — blocks Phase B | `FAIL` → repair/replan (semantic change) |
| Frozen capture rule honored in live frames | CORE | DIRECT | YES | any INVALID unexplained frame → abort |
| Postcondition direct observation (frozen predicate, bounded window) | CORE | DIRECT | YES | affirmative/no-affirmative/late per §10 names |
| Chooser identity + exactly-once confirmation | CORE | DIRECT | YES | refusal → `INDETERMINATE_CHOOSER_REFUSED` |
| Staging predicate (57/stable/decodable/17,924,900 B/multiset) | CORE | DIRECT | YES | named mismatch classes (§12) |
| Baseline unchanged (digests + mtimes) | CORE | DIRECT | YES | baseline delta → immediate abort + report |
| Replay/adversarial suites | SUPPORTING | SUPPORTING | NO (unless a CORE item depends on it) | failure → repair before Phase B |
| Reviews (7 topics + adversarial + barrier supersession) | CORE | DIRECT | YES — blocks Phase B | `REVISION_REQUIRED` → repair+re-review |
| Stage-05 independent acceptance | CORE | DIRECT | YES — blocks TASK_CLOSURE | recompute-from-disk mismatch → fail/replan |
| Preservation audit (do-not-touch, no pycache, no push) | SUPPORTING | SUPPORTING | NO | violation → stop + report |

`CHECK_RESULT`/`WAIVER_STATUS` per item are recorded in `execution.md` (Stage 04) and the Stage-05 report; the final report carries the v4.2 subjects separately (PRIMARY_OUTCOME / IMPLEMENTATION / CORE_ACCEPTANCE / REQUIRED_VERIFICATION / INDEPENDENT_ACCEPTANCE / TASK_CLOSURE) with scoped blockers, never a naked `BLOCKED` and never `DONE` from synthetic evidence.
...
(b) a scoped, honestly-reported terminal state — `CHOOSER_OBSERVED_AFTER_WINDOW`, `NO_CHOOSER_OBSERVED`, `CONTENT_MISMATCH_AGAINST_ACCEPTED_BASELINE`, `STAGING_INCOMPLETE`/`STAGING_EXTRA_FILES`/`STAGING_DUPLICATE_CONTENT`/`STAGING_UNSTABLE`, `ABORTED_*`, `INDETERMINATE_*`, or `BLOCKED_WITH_ROOT_CAUSE` (only after the audit threshold) — with all evidence preserved.
