...
Writes s11c-final-gate.json. Never sends GUI input. Never modifies frozen artifacts.
"""
...
FRAME = "s11c-preclick-frame.png"
DET_OUT = "s11c-menu-detect.json"
GEO_OUT = "s11c-menu-frame-geometry.json"
V8_OUT = "s11c-v8-locate-save-all.json"
GATE_OUT = "s11c-final-gate.json"
...
def main():
    ...
    axp = os.path.join(ATT, "s11c-preclick-ax.json")
    ax = json.load(open(axp))
    gate["fresh_ax"] = {"path": "s11c-preclick-ax.json", "sha256": sha256_file(axp),
    ...
    geo = {"artifact": GEO_OUT, "frame_sha256": fsha, "capture_scale": 2,
           ...
           "provenance": f"fresh read-only AX read at {now} (s11c-preclick-ax.json): detail window main+focused, "
    ...
    binv = {"artifact": "baseline-inventory-s11c-preclick.json", "recorded_at_local": now,
            ...
            **inv, "expected_digest": EXP["baseline_digest"],
            ...
            "gui_input_count": 1}
    open(os.path.join(ATT, "baseline-inventory-s11c-preclick.json"), "w").write(json.dumps(binv, ensure_ascii=False, indent=1) + "\n")
