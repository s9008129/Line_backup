#!/usr/bin/env python3
"""S11b pre-click freshness re-validation for the Save All dispatch (read-only; ZERO GUI input).
...
Writes s11b-final-gate.json. Never sends GUI input. Never modifies frozen artifacts.
"""
...
FRAME = "s11b-preclick-frame.png"
DET_OUT = "s11b-menu-detect.json"
GEO_OUT = "s11b-menu-frame-geometry.json"
V8_OUT = "s11b-v8-locate-save-all.json"
GATE_OUT = "s11b-final-gate.json"
...
def main():
    ...
    gate = {"artifact": GATE_OUT, "phase": "S11b_final_freshness_gate_at_dispatch_moment", "attempt": "07",
    ...
    axp = os.path.join(ATT, "s11b-preclick-ax.json")
    ax = json.load(open(axp))
    gate["fresh_ax"] = {"path": "s11b-preclick-ax.json", "sha256": sha256_file(axp),
    ...
    geo = {"artifact": GEO_OUT, "frame_sha256": fsha, "capture_scale": 2,
           ...
           "provenance": f"fresh read-only AX read at {now} (s11b-preclick-ax.json): detail window main+focused, "
                         f"position {EXP['window_pos']} size {EXP['window_size']}; bound to the S11b fresh frame sha; NOT historical geometry",
           "gui_inputs_sent": 1}
    ...
    binv = {"artifact": "baseline-inventory-s11b-preclick.json", "recorded_at_local": now,
            ...
            **inv, "expected_digest": EXP["baseline_digest"],
            ...
            "gui_input_count": 1}
    open(os.path.join(ATT, "baseline-inventory-s11b-preclick.json"), "w").write(json.dumps(binv, ensure_ascii=False, indent=1) + "\n")
