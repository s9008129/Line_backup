#!/usr/bin/env python3
"""Self-test matrix for the v4 route tools (macOS-Vision reader), 16 cases.

Cases 1-13 re-express the frozen v3 self-test matrix (selftest/v3) against the v4
...
TOOLS = os.path.normpath(os.path.join(HERE, os.pardir, os.pardir, "v4"))
V3_SUMMARY = os.path.normpath(os.path.join(HERE, os.pardir, "v3", "selftest-summary.json"))
V3_SUMMARY_SHA256_PINNED = "17840e915680308fb721a937462d22898c3adb1aa125e3cb2469f74c31344324"
V4_TOOL_FILES = ("vision_reader.py", "locate_album_card.py", "verify_album_open.py",
                 "locate_album_ellipsis.py")
FAILCLOSED_BIN = "/nonexistent/vision_ocr_v4_selftest"
W, H = 327, 643
...
def missing_tools():
    return [name for name in V4_TOOL_FILES
            if not os.path.isfile(os.path.join(TOOLS, name))]


def tool_shas():
    return {name: sha256_file(os.path.join(TOOLS, name)) for name in V4_TOOL_FILES}
...
def fixed_build_binary_path():
    ...
    try:
        spec = importlib.util.spec_from_file_location("v4_vision_reader_selftest", path)
...
def main():
    ...
    if missing:
        print(json.dumps({"tool": "v4-selftest", "result": "BLOCKED",
                          "reason": "v4 tool files missing", "missing": missing,
                          "tools_dir": TOOLS}, ensure_ascii=False, indent=1))
        return 3

    shas_before = tool_shas()
    v3_summary_sha = sha256_file(V3_SUMMARY) if os.path.exists(V3_SUMMARY) else None

    fixtures = tempfile.mkdtemp(prefix="v4selftest_")
    interpreter = sys.executable
    summary = {"tool": "v4-selftest", "interpreter": interpreter,
    ...
    if not tools_stable:
        failures += 1
    v3_summary_sha_after = sha256_file(V3_SUMMARY) if os.path.exists(V3_SUMMARY) else None
    v3_unchanged = v3_summary_sha_after == V3_SUMMARY_SHA256_PINNED
    if not v3_unchanged:
        failures += 1
    ...
    summary["expectations_pinned_to"] = {
        "v3_summary_path": V3_SUMMARY,
        "v3_summary_sha256_pinned": V3_SUMMARY_SHA256_PINNED,
        "v3_summary_sha256_at_run_start": v3_summary_sha,
        "v3_summary_sha256_after_run": v3_summary_sha_after,
        "v3_summary_unchanged": v3_unchanged,
        "v3_case_literals": ("cases 1-13 carry the frozen v3 summary's expect_exit/expect_verdict "
                             "verbatim; the two ellipsis control cases keep the v3 click_point "
                             "[300,47] with the +-2 tolerance"),
        "v4_reader_layer_literals": ("cases 14-16 are new reader-layer expectations from plan Rev21 "
