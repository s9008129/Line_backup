...
final class App: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ n: Notification) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.2) { self.probe() }
    }
    func probe() {
        ...
        Task { @MainActor in
            do {
                ...
                print("windowNumber=\(self.win.windowNumber); SC windows: " + content.windows.map { "\($0.windowID):\($0.frame)" }.joined(separator: " | "))
                ...
                for (label, shadows, clipping) in [("default", false, false), ("ignoreShadows", true, false), ("ignoreClipping", false, true), ("both", true, true)] {
                    let f = SCContentFilter(desktopIndependentWindow: mw)
                    let c = SCScreenshotConfiguration(); c.includeChildWindows = false; c.showsCursor = false
                    c.ignoreShadows = shadows; c.ignoreClipping = clipping
                    do {
                        let o = try await SCScreenshotManager.captureScreenshot(contentFilter: f, configuration: c)
                        if let img = o.sdrImage {
                            let rep = NSBitmapImageRep(cgImage: img)
                            let mag = clusters(rep) { $0.redComponent > 0.7 && $0.blueComponent > 0.7 && $0.greenComponent < 0.4 }
                            print("cfg=\(label) px=\(img.width)x\(img.height) pt=\(Double(img.width)/2)x\(Double(img.height)/2) magenta=\(mag.prefix(2))")
                        }
                    } catch { print("cfg=\(label) ERROR \(error)") }
                }
